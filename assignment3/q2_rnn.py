...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence)<max_length:
          ...
          newsentence=sentence+[zero_vector]*padding_length
          newlabels=labels+[zero_label]*padding_length
          mask=[True]*len(sentence)+[False]*padding_length
        else:
          newsentence=sentence[:max_length]
          newlabels=labels[:max_length]
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE (~4-6 lines)
        U=tf.get_variable("U", shape=(Config.hidden_size, Config.n_classes), dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
        b_2=tf.get_variable("b_2", shape=(Config.n_classes,), dtype=tf.float32, initializer=tf.zeros_initializer())
        h_t=tf.zeros((1, Config.hidden_size), dtype=tf.float32)
        ### END YOUR CODE
