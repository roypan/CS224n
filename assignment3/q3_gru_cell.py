...
class GRUCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            #o_t = tanh(x_t U_o + r_t * h_{t-1} W_o + b_o)
            #h_t = z_t * h_{t-1} + (1 - z_t) * o_t
            U_r=tf.get_variable("U_r", shape=(self.input_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32)
            W_r=tf.get_variable("W_r", shape=(self.state_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32)
            b_r=tf.get_variable("b_r", shape=self.state_size, initializer=tf.zeros_initializer(), dtype=tf.float32)
            U_z=tf.get_variable("U_z", shape=(self.input_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32)
            W_z=tf.get_variable("W_z", shape=(self.state_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32)
            b_z=tf.get_variable("b_z", shape=self.state_size, initializer=tf.zeros_initializer(), dtype=tf.float32)
            U_o=tf.get_variable("U_o", shape=(self.input_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32)
            W_o=tf.get_variable("W_o", shape=(self.state_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32)
            b_o=tf.get_variable("b_o", shape=self.state_size, initializer=tf.zeros_initializer(), dtype=tf.float32)
